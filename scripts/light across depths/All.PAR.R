...
##########################################
###### June - July Deployment ############
##########################################

##### grab files in a list
PAR.files <- list.files(path="data/environmental/temp and light/Jun_DecPAR/light_T11", pattern = "CSV$", full.names = T); PAR.files

##### what are the file names, sans extensions
file.names<-file_path_sans_ext(list.files(path="data/environmental/temp and light/Jun_DecPAR/light_T11", pattern = "CSV$", full.names = F))
...
June.PAR<-as.data.frame(c(files[, data_index])) # here is the data we want, now in df alone
