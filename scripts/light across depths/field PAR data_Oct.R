...
#####################
data<-read.csv("data/cross depth PAR temp/Oct 2016/6377_logger5_Oct2016.csv", skip=8)
df<-data[, c(-1,-5)] # removes "trash" columns
colnames(df)<-c("Date", "Time", "Raw.PAR")
df$Date<-parse_date_time(df$Date, "dmy") # corrects date format
df$timestamp<-as.POSIXct(paste(df$Date, df$Time), format="%Y-%m-%d %H:%M:%S")
...
# Light logger 6: SN 6379
#####################
data<-read.csv("data/cross depth PAR temp/Oct 2016/6379_logger6_Oct2016.csv", skip=8)
df<-data[, c(-1,-5)] # removes "trash" columns
colnames(df)<-c("Date", "Time", "Raw.PAR")
df$Date<-parse_date_time(df$Date, "dmy") # corrects date format
df$timestamp<-as.POSIXct(paste(df$Date, df$Time), format="%Y-%m-%d %H:%M:%S")
...
df<-df[!(df$timestamp < "2016-10-07 14:00:00"),] # start at this time
df<-df[!(df$timestamp >= "2016-10-19 17:00:00"),] # end at this time
# df$Calibrated.PAR<-(df$Raw.PAR*xxx) # from calibration file??
log6_6379<-df
...
# Light logger Raph: SN 4323
#####################
data<-read.csv("data/cross depth PAR temp/Oct 2016/Rf25_Oct3_4323.CSV", skip=8)
df<-data[, c(-1,-5)] # removes "trash" columns
colnames(df)<-c("Date", "Time", "Raw.PAR")
df$Date<-parse_date_time(df$Date, "dmy") # corrects date format
df$timestamp<-as.POSIXct(paste(df$Date, df$Time), format="%Y-%m-%d %H:%M:%S")
...
df<-df[!(df$timestamp < "2016-10-07 14:00:00"),] # start at this time
df<-df[!(df$timestamp >= "2016-10-19 17:00:00"),] # end at this time
df$Calibrated.PAR<-(df$Raw.PAR*0.104) # from calibration file
plot(df$Calibrated.PAR)
log9_4323<-df


# Light logger Raph: SN 4322
#####################
...
## combine dataframes
ALL.PAR<-cbind(log1_2484[c(1,3)], log3_6378[c(0,3)], log4_2488[c(0,3)], log5_6377[c(0,3)], log7_4807[c(0,3)], log8_4804[c(0,3)], log9_4323[c(0,3)], log10_4322[c(0,3)], log11_4375[c(0,3)], log12_4805[c(0,3)])

colnames(ALL.PAR)<-c("timestamp", "log1_2484_CalPAR","log3_6378_CalPAR", "log4_2488_CalPAR", "log5_6377_CalPAR", "log7_4807_CalPAR", "log8_4804_CalPAR", "log9_4323_CalPAR","log10_4322_CalPAR", "log11_4375_CalPAR", "log12_4805_CalPAR")
...
colnames(ALL.PAR)
colnames(ALL.PAR)<-c("timestamp", "Rf44.shallow", "Rf42.deep", "Rf42.shallow", "Rf25.shallow", "HIMB.shallow", "HIMB.deep", "Rf25.mid", "Rf42.mid", "Rf44.mid", "HIMB.mid")

# reorder columns from N>S and shallow to deep
ALL.PAR<-ALL.PAR[, c(1:2,10,4,9,3,5,8,6,11,7)]
colnames(ALL.PAR)
...
df.dli<-aggregate(data.frame(Rf44.shall.DLI=df.split[[1]]$Rf44.shallow*0.0864,
                         ...
                         Rf25.shall.DLI=df.split[[1]]$Rf25.shallow*0.0864,
                         Rf25.mid.DLI=df.split[[1]]$Rf44.mid*0.0864,
...
#########################
# Reef 25
plot(DLI ~ Date, Timespan.df, type="n", ylab=(expression(paste("DLI mol photons", ~m^-2, ~d^-1, sep=""))), ylim=c(0, 55), xaxt="n", xlab="",  main="Reef 25 PAR")
mtext(expression(bold("C")), 2, adj=-1, las=1, padj=-10.5)
axis.Date(1, at=seq(min(Timespan.df$Date), max(Timespan.df$Date), by="1 day"), format="%m/%d/%y")
legend("topright", lty=1, col=c(reefcols[1:3]), legend=c("< 1 m","2 m", "7 m"), lwd=3, bty="n")

with(na.omit(data.frame(date=df.dli$Date, DLI=rollmean(df.dli$Rf25.shall.DLI, k, fill=NA))), { 
  lines(date, DLI, col=reefcols[1], lwd=2) 
})
with(na.omit(data.frame(date=df.dli$Date, DLI=rollmean(df.dli$Rf25.mid.DLI, k, fill=NA))), { 
  lines(date, DLI, col=reefcols[2], lwd=2) 
})
