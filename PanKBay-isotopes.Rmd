...
### Light at Depth
```{r light at depth, fig.dim=c(6,5), fig.align='center'}
########################## # LIGHT AT DEPTH
#############
###### 
...
```{r isotope sw figures boxplot1, results="hide", fig.cap="*Figures of size-fractioned sample isotope values stratified by seasons (summer and winter)*"}
...
```{r isotope sw figures boxplot2, results="hide", fig.cap="*Figures of isotope values in seawater particles stratified by sites (northwest to southeast)*"}
######## Sites
op<-par(mfrow = c(2,2), mar=c(5,5,2,1))
...
```
```{r SW isotope figure, results="hide", fig.cap="*Isotope values for size-fractioned seawater particles and plankters pooled across 2 seasons and 6 reef sites*"}
...
dev.copy(pdf, "output/iso.sources.KBay.pdf", encod="MacRoman", height=4, width=4)
dev.off()
```
