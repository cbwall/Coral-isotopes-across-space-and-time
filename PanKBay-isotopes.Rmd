...
# figures
#########################
pdf(paste("figures/environmental/Temp.allsites",".pdf", sep=""))
...
plot(Rf42.mean ~ Date, df.mean, type="n", ylab="Temperature (°C)", ylim=c(23, 30), xaxt="n", xlab="")
 axis.Date(1, at=seq(min(df.mean$Date), max(df.mean$Date), by="1 mo"), format="%m/%d/%y", cex.axis=0.8)
...
legend("topright", lty=1, col=c(reefcols[1:4]), legend=c("NW","NE", "SW", "SE"), lwd=3, bty="n")
