...
```{r SPM models and figrues, fig.dim=c(6,3.5)}
###### SPM
...
mod<-(lm(d13C~Location+Time.point+SW.fraction..um, data=SWiso)); Anova(mod, type=2) # no location effect
mod<-(lm(d15N~Location+Time.point+SW.fraction..um, data=SWiso)); Anova(mod, type=2) # no location effect
...
```{r qPCR figures, results="hide",fig.dim=c(4,4), fig.cap="**Figure** Probability of *Montipora capitata* hosting clade D symbionts across a depth gradient in summer and winter seasons. Lines represent model fit to data by logistic regression for summer alone, winter alone, and combined (summer + winter) data"}
...
## **Note** where points equal 0.0 is 100% D, where they equal 0 is 100% C
par(mar=c(5,4,3,2))
plot(sum.dat$propD~sum.dat$Light, xlab="Depth (m)", ylab = "Proportion of Clade D Symbiont", pch=19, col="salmon", xlim=c(0,25), ylim=c(0.0, 1.0), cex=0.5, cex.lab=0.8)
par(new=T)
