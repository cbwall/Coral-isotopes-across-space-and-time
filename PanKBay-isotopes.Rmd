...
mod<-lmer(DLI~Location*Season + (1|Date), data=All.DLI.mod); print(anova(mod, type=2), digits=7)
plot(allEffects(mod))
...
```{r SPM models and figures, fig.dim=c(6,3.5), eval=FALSE}
#### Gravimetric SPM
...
posthoc<-emmeans(mod, ~Location|Date)
CLD(posthoc, Letters=letters)
...
spm.n<-aggregate(SPM.mg~Date+Location+Reef, spm, length)
