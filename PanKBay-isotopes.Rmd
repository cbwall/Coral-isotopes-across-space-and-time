...
We measured changes in total biomass (ash-free dry weight), symbiont densiteis, photopigments (total chlorophylls (*a+c~2~*)), and stable isotopes (δ^13^C, δ^15^N) of the coral host, its endosymbionts Symbiodiniaceae, and the coral skeleton.  
...
### Temperature
```{r, temp, results=FALSE, fig.cap="**Figure** Temperature across sites and time"}
...
# figures
#########################
pdf(paste("figures/environmental/Temp.allsites",".pdf", sep=""), height=3.5, width=6)
...
```{r light as a factor figure, results='hide', fig.show='hide'}
...
##### figure PAR at 3 depths, results='hide', fig.cap="**Figure** *PAR at the four reef areas from summer to winter 2016 at three depths (<1m, 2m, 8m). Data at 2m was recorded from loggers; data at <1m and 8m were modeled using light attenuation calculations*"
...
```{r bar plot of DLI at depths, fig.dim=c(4,4), fig.cap="**Figure** Average (+/- SE) Daily Light Integral (DLI) for four reef locations at three depths pooled from June - January. DLI estimated from light attenuation coefficents using depth as a fixed effect against baseline loggers at 2 m depth"}
...
kd.all # kds for each site, depth as continuous
...
plot(Light~newDepth, Sum, col="gray30", bg="gray40", pch=21, xlab="Depth (m)", ylab="DLI at coral", 
     ylim=c(0, 30), xlim=c(0, 10), cex=0.7,  cex.lab=0.7, cex.axis=0.7)
...
curve(exp(coef(wintmod)["(Intercept)"]+coef(wintmod)["newDepth"]*x), add=TRUE, col="gray80", 
      lwd=2, xlim=c(0,7.8))
legend("topright", legend=c("Summer", "Winter"), col=c("gray30", "gray60"), pt.bg=c("gray40", "gray80"), lty=c(1,1), lwd=c(1,1), pch=c(21,21), bty="n", 
       y.intersp = 1.5, x.intersp = 0.4, inset=c(0.1, 0))
dev.copy(pdf, "figures/symbionts/Light by Depth_symbs.pdf", encod="MacRoman", height=3, width=4)
dev.off()
```
...
legend("topleft", legend=Locations, col=plot_colors[1:4], lwd=1, pch=19, lty=2, cex=0.8, bty="n", x.intersp=0.2, y.intersp=2, inset=c(0.01, 0), seg.len=1.5)
...
#anova(results.win, test = "Chisq")
#summary(results.win)
fitted.win <- predict(results.win, newdata = list(Light=seq(0,14,0.1)), type = "response")
# plot(fitted.win, ylab="proportion D", ylim=c(0,1))
...
## **Note** where points equal 0.0 is 100% D, where they equal 0 is 100% C
par(mar=c(5,4,3,2))
plot(sum.dat$propD~sum.dat$Light, xlab="Light (DLI)", ylab = "Proportion of Durusdinium", pch=21, col="gray30", bg="gray40", xlim=c(25,0), ylim=c(0.0, 1.0), cex=0.7, cex.lab=0.7, cex.axis=0.7)
par(new=T)
plot(wint.dat$propD~wint.dat$Light, pch=21, xlim=c(25,0), ylim=c(0.0, 1.0), xaxt="n", yaxt="n",
     col="gray60", bg="gray80", xlab="", ylab="", cex=0.7)
lines(fitted.all ~ seq(0,25,0.1), col="black", lwd=1, lty=2)
lines(fitted.sum ~ seq(0,25,0.1), col="gray40", lwd=1)
lines(fitted.win ~ seq(0,14,0.1), col="gray80", lwd=1)
legend("bottomleft", pch=c(19,19, NA), lty=c(1,1,2), col=c("gray30", "gray60", "black"), pt.bg=c("gray40", "gray80", NA), legend=c("Summer", "Winter", "Combined"), lwd=1, bty="n", x.intersp=1, pt.cex=0.7, y.intersp=2, cex=0.5, inset=c(0.1, 0.15), seg.len=4)

dev.copy(pdf, "figures/symbionts/Symbionts_by_Season_light.pdf", encod="MacRoman", height=3, width=4)
dev.off()
...
### Figures
```{r PCA all data, result="hide", fig.dim=c(7,6)}
...
ggsave(file="figures/multivariate/PCx4.pdf", G, height=6, width=6)
...
legend("topright", c("", ""), pch=c(21,21), pt.bg=c("salmon", "deepskyblue1"), col=c("red3", "blue"), cex=0.8, y.intersp = 0.9, x.intersp = 0.4, bty="n", inset=c(0.345, 0))
par(new=T)
...
# C summer plot
plot(host..d13C~zoox, data=C.sum.df, col='red3', ylim=c(-20, -12), xlim=c(0, 6000000), cex=0.7, pch=21, bg="salmon", main="Summer", cex.main=0.7,
     xlab=expression(paste("symbionts ", "(",cells~cm^-2~")", sep="")), 
     ylab=(expression(paste(delta^{13}, C[H], " (\u2030, V-PDB)"))), cex.axis=0.8,
     cex.lab=0.8)
Axis(side=2, at = seq(-20, -12, by = 2), cex.axis=0.8)
legend("topright", c("clade C", "clade D"), lty=c(1,1), lwd=c(2,2), seg.len=0.7, col=c("salmon", "deepskyblue1"), cex=0.8,
       y.intersp = 0.9, x.intersp = 0.4, bty="n", inset=c(-0.4, -0.03))
...
ablineclip(lm(host..d13C~zoox, data=C.sum.df), col='salmon', x1 = min(C.sum.df$zoox), x2 = max(C.sum.df$zoox), lwd=2)

# D summer plot
par(new=T)
plot(host..d13C~zoox, data=D.sum.df, col='blue', ylim=c(-20, -12), xlim=c(0, 6000000), cex=0.7, pch=21, bg="deepskyblue1", xaxt="n", yaxt="n", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(host..d13C~zoox, data=D.sum.df), col='deepskyblue1', x1 = min(D.sum.df$zoox), x2 = max(D.sum.df$zoox), lwd=2)

# C winter plot
plot(host..d13C~zoox, data=C.win.df, col='red3', ylim=c(-20, -12), xlim=c(0, 6000000), cex=0.7, pch=21, bg="salmon", xaxt="n", yaxt="n", main="Winter", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(host..d13C~zoox, data=C.win.df), col='salmon', x1 = min(C.win.df$zoox), x2 = max(C.win.df$zoox), lwd=2)

# D winter plot
par(new=T)
plot(host..d13C~zoox, data=D.win.df, col='blue', ylim=c(-20, -12), xlim=c(0, 6000000), cex=0.7, pch=21, bg="deepskyblue1", ylab="", cex.main=0.7, yaxt="n",
xlab=expression(paste("symbionts ", "(",cells~cm^-2~")", sep="")), cex.axis=0.8,
     cex.lab=0.8)
Axis(side=2, labels=FALSE)
ablineclip(lm(host..d13C~zoox, data=D.win.df), col='deepskyblue1', x1 = min(D.win.df$zoox), x2 = max(D.win.df$zoox), lwd=2)
...
# C summer plot
plot(symb..d13C~zoox, data=C.sum.df, col='red3', ylim=c(-20, -12), xlim=c(0, 6000000), cex=0.7, pch=21, bg="salmon", main="Summer", cex.main=0.7,
     xlab=expression(paste("symbionts ", "(",cells~cm^-2~")", sep="")), 
     ylab=(expression(paste(delta^{13}, C[S], " (\u2030, V-PDB)"))), cex.axis=0.8,
     cex.lab=0.8)
Axis(side=2, at = seq(-20, -12, by = 2), cex.axis=0.8)
...
# D summer plot
par(new=T)
plot(symb..d13C~zoox, data=D.sum.df, col='blue', ylim=c(-20, -12), xlim=c(0, 6000000), cex=0.7, pch=21, bg="deepskyblue1", xaxt="n", yaxt="n", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(symb..d13C~zoox, data=D.sum.df), col='deepskyblue1', x1 = min(D.sum.df$zoox), x2 = max(D.sum.df$zoox), lwd=2)

# C winter plot
plot(symb..d13C~zoox, data=C.win.df, col='red3', ylim=c(-20, -12), xlim=c(0, 6000000), cex=0.7, pch=21, bg="salmon", xaxt="n", yaxt="n", main="Winter", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(symb..d13C~zoox, data=C.win.df), col='salmon', x1 = min(C.win.df$zoox), x2 = max(C.win.df$zoox), lwd=2)

# D winter plot
par(new=T)
plot(symb..d13C~zoox, data=D.win.df, col='blue', ylim=c(-20, -12), xlim=c(0, 6000000), cex=0.7, pch=21, bg="deepskyblue1", ylab="", cex.main=0.7, yaxt="n",
xlab=expression(paste("symbionts ", "(",cells~cm^-2~")", sep="")), cex.axis=0.8,
     cex.lab=0.8)
Axis(side=2, at = seq(-20, -12, by = 2), cex.axis=0.8)
...
# C summer plot
plot(host..d13C~chltot, data=C.sum.df, col='red3', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="salmon", main="Summer", cex.main=0.7,
...
legend("topright", c("clade C", "clade D"), lty=c(1,1), lwd=c(2,2), seg.len=0.7, col=c("salmon", "deepskyblue1"), cex=0.8,
       y.intersp = 0.9, x.intersp = 0.4, bty="n", inset=c(-0.4, -0.03))
legend("topright", c("", ""), pch=c(21,21), pt.bg=c("salmon", "deepskyblue1"), col=c("red3", "blue"), cex=0.8, y.intersp = 0.9, x.intersp = 0.4, bty="n", inset=c(0.3, -0.03))
ablineclip(lm(host..d13C~chltot, data=C.sum.df), col='salmon', x1 = min(C.sum.df$chltot), x2 = max(C.sum.df$chltot), lwd=2)

# D summer plot
par(new=T)
plot(host..d13C~chltot, data=D.sum.df, col='blue', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="deepskyblue1", xaxt="n", yaxt="n", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(host..d13C~chltot, data=D.sum.df), col='deepskyblue1', x1 = min(D.sum.df$chltot), x2 = max(D.sum.df$chltot), lwd=2)

# C winter plot
plot(host..d13C~chltot, data=C.win.df, col='red3', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="salmon", xaxt="n", yaxt="n", main="Winter", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(host..d13C~chltot, data=C.win.df), col='salmon', x1 = min(C.win.df$chltot), x2 = max(C.win.df$chltot), lwd=2)

# D winter plot
par(new=T)
plot(host..d13C~chltot, data=D.win.df, col='blue', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="deepskyblue1", ylab="", cex.main=0.7, yaxt="n",
xlab=expression(paste("total chlorophyll", ~(mu*g~cm^-2), sep="")), cex.axis=0.8,
     cex.lab=0.8)
Axis(side=2, labels=FALSE)
ablineclip(lm(host..d13C~chltot, data=D.win.df), col='deepskyblue1', x1 = min(D.win.df$chltot), x2 = max(D.win.df$chltot), lwd=2)

dev.copy(pdf, "figures/regressions/PanKB.chltot.d13Chost.pdf", width=4, height=4, encod="MacRoman")
dev.off()
...
# C summer plot
plot(symb..d13C~chltot, data=C.sum.df, col='red3', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="salmon", main="Summer", cex.main=0.7,
     xlab=expression(paste("total chlorophyll", ~(mu*g~cm^-2), sep="")),
     ylab=(expression(paste(delta^{13}, C[S], " (\u2030, V-PDB)"))), cex.axis=0.8,
     cex.lab=0.8)
legend("topright", c("clade C", "clade D"), lty=c(1,1), lwd=c(2,2), seg.len=0.7, col=c("salmon", "deepskyblue1"), cex=0.8,
...
# D summer plot
par(new=T)
plot(symb..d13C~chltot, data=D.sum.df, col='blue', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="deepskyblue1", xaxt="n", yaxt="n", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(symb..d13C~chltot, data=D.sum.df), col='deepskyblue1', x1 = min(D.sum.df$chltot), x2 = max(D.sum.df$chltot), lwd=2)

# C winter plot
plot(symb..d13C~chltot, data=C.win.df, col='red3', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="salmon",  yaxt="n", main="Winter", ylab="", xlab ="", cex.main=0.7, cex.axis=0.8)
ablineclip(lm(symb..d13C~chltot, data=C.win.df), col='salmon', x1 = min(C.win.df$chltot), x2 = max(C.win.df$chltot), lwd=2)

# D winter plot
par(new=T)
plot(symb..d13C~chltot, data=D.win.df, col='blue', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="deepskyblue1", ylab="", cex.main=0.7, yaxt="n", xaxt="n",
xlab=expression(paste("total chlorophyll", ~(mu*g~cm^-2), sep="")), cex.axis=0.8,
     cex.lab=0.8)
Axis(side=2, labels=FALSE)
...
# C summer plot
plot(host..d13C~chlcell, data=C.sum.df, col='red3', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="salmon", main="Summer", cex.main=0.7, 
...
legend("topright", c("clade C", "clade D"), lty=c(1,1), lwd=c(2,2), seg.len=0.7, col=c("salmon", "deepskyblue1"), cex=0.8,
       y.intersp = 0.9, x.intersp = 0.4, bty="n", inset=c(-0.4, -0.03))
legend("topright", c("", ""), pch=c(21,21), pt.bg=c("salmon", "deepskyblue1"), col=c("red3", "blue"), cex=0.8, y.intersp = 0.9, x.intersp = 0.4, bty="n", inset=c(0.3, -0.03))
ablineclip(lm(host..d13C~chlcell, data=C.sum.df), col='salmon', x1 = min(C.sum.df$chlcell), x2 = max(C.sum.df$chlcell), lwd=2)

# D summer plot
par(new=T)
plot(host..d13C~chlcell, data=D.sum.df, col='blue', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="deepskyblue1", xaxt="n", yaxt="n", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(host..d13C~chlcell, data=D.sum.df), col='deepskyblue1', x1 = min(D.sum.df$chlcell), x2 = max(D.sum.df$chlcell), lwd=2)

# C winter plot
plot(host..d13C~chlcell, data=C.win.df, col='red3', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="salmon", xaxt="n", main="Winter", ylab="", xlab ="", cex.main=0.7, cex.axis=0.8)
ablineclip(lm(host..d13C~chlcell, data=C.win.df), col='salmon', x1 = min(C.win.df$chlcell), x2 = max(C.win.df$chlcell), lwd=2)

# D winter plot
par(new=T)
plot(host..d13C~chlcell, data=D.win.df, col='blue', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="deepskyblue1", ylab="", cex.main=0.7, yaxt="n",
xlab=(expression(paste("chlorophyll (",pg~cell^-1, ")", sep=""))), cex.axis=0.8, cex.lab=0.8)
ablineclip(lm(host..d13C~chlcell, data=D.win.df), col='deepskyblue1', x1 = min(D.win.df$chlcell), x2 = max(D.win.df$chlcell), lwd=2)
...
# C summer plot
plot(symb..d13C~chlcell, data=C.sum.df, col='red3', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="salmon", main="Summer", cex.main=0.7,
     xlab=(expression(paste("chlorophyll (",pg~cell^-1, ")", sep=""))),
     ylab=(expression(paste(delta^{13}, C[S], " (\u2030, V-PDB)"))), cex.axis=0.8,
     cex.lab=0.8)
legend("topright", c("clade C", "clade D"), lty=c(1,1), lwd=c(2,2), seg.len=0.7, col=c("salmon", "deepskyblue1"), cex=0.8,
...
# D summer plot
par(new=T)
plot(symb..d13C~chlcell, data=D.sum.df, col='blue', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="deepskyblue1", xaxt="n", yaxt="n", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(symb..d13C~chlcell, data=D.sum.df), col='deepskyblue1', x1 = min(D.sum.df$chlcell), x2 = max(D.sum.df$chlcell), lwd=2)

# C winter plot
plot(symb..d13C~chlcell, data=C.win.df, col='red3', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="salmon", xaxt="n", yaxt="n", main="Winter", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(symb..d13C~chlcell, data=C.win.df), col='salmon', x1 = min(C.win.df$chlcell), x2 = max(C.win.df$chlcell), lwd=2)

# D winter plot
par(new=T)
plot(symb..d13C~chlcell, data=D.win.df, col='blue', ylim=c(-20, -12), xlim=c(0, 12), cex=0.7, pch=21, bg="deepskyblue1", ylab="", cex.main=0.7,
...
# C summer plot
plot(d13C..host.sym~zoox, data=C.sum.df, col='red3', ylim=c(-2, 2), xlim=c(0, 6000000), cex=0.7, pch=21, bg="salmon", main="Summer", cex.main=0.7,
     xlab=expression(paste("symbionts ", "(",cells~cm^-2~")", sep="")), 
     ylab=(expression(paste(delta^{13}, C[H-S], " (\u2030, V-PDB)"))), cex.axis=0.8,
     cex.lab=0.8)
legend("topright", c("clade C", "clade D"), lty=c(1,1), lwd=c(2,2), seg.len=0.7, col=c("salmon", "deepskyblue1"), cex=0.8,
       y.intersp = 0.9, x.intersp = 0.4, bty="n", inset=c(-0.4, -0.03))
legend("topright", c("", ""), pch=c(21,21), pt.bg=c("salmon", "deepskyblue1"), col=c("red3", "blue"), cex=0.8, y.intersp = 0.9, x.intersp = 0.4, bty="n", inset=c(0.3, -0.03))
ablineclip(lm(d13C..host.sym~zoox, data=C.sum.df), col='salmon', 
           x1 = min(C.sum.df$zoox), x2 = max(C.sum.df$zoox), lwd=2)

# D summer plot
par(new=T)
plot(d13C..host.sym~zoox, data=D.sum.df, col='blue', ylim=c(-2, 2), xlim=c(0, 6000000), cex=0.7, pch=21, bg="deepskyblue1", xaxt="n", yaxt="n", ylab="", xlab ="", cex.main=0.7)
ablineclip(lm(d13C..host.sym~zoox, data=D.sum.df), col='deepskyblue1', x1 = min(D.sum.df$zoox), x2 = max(D.sum.df$zoox), lwd=2)

# C winter plot
plot(d13C..host.sym~zoox, data=C.win.df, col='red3', ylim=c(-2, 2), xlim=c(0, 6000000), cex=0.7, pch=21, bg="salmon", main="Winter", ylab="", xlab ="", cex.main=0.7, cex.axis=0.8)
ablineclip(lm(d13C..host.sym~zoox, data=C.win.df), col='salmon', x1 = min(C.win.df$zoox), x2 = max(C.win.df$zoox), lwd=2)

# D winter plot
par(new=T)
plot(d13C..host.sym~zoox, data=D.win.df, col='blue', ylim=c(-2, 2), xlim=c(0, 6000000), cex=0.7, pch=21, bg="deepskyblue1", ylab="", cex.main=0.7, yaxt="n", xaxt="n",
xlab=expression(paste("symbionts ", "(",cells~cm^-2~")", sep="")), cex.axis=0.8, cex.lab=0.8)
ablineclip(lm(d13C..host.sym~zoox, data=D.win.df), col='deepskyblue1', x1 = min(D.win.df$zoox), x2 = max(D.win.df$zoox), lwd=2)
